...
export default function RealmDetailScreen() {
  ...
  return (
    <Screen>
      ...
      <Text style={[typeScale.display, { color: colors.ink }]}>{realm.name}</Text>
      <Text style={[typeScale.body, { color: colors.dim }]}>{realm.tagline}</Text>

      <Card accent={colors.gold}>
        <Text style={[typeScale.label, { color: colors.gold }]}>GUARDIAN</Text>
        <Text style={[typeScale.h1, { color: colors.ink }]}>{realm.guardian.name}</Text>
      ...
      <Card accent={realm.accent}>
        <Text style={[typeScale.label, { color: colors.gold }]}>TRANSCENDENCE</Text>
      ...
      <Text style={[typeScale.h2, { color: colors.ink }]}>Trials</Text>
      {realm.trials.map((t) => (
        <Card key={t.id}>
          <Badge label={t.kind} tone="dim" />
          <Text style={[typeScale.body, { color: colors.ink, fontWeight: '700' }]}>{t.title}</Text>
          <Text style={[typeScale.caption, { color: colors.dim }]} numberOfLines={2}>
            {t.prompt}
          </Text>

...
export default function TrialScreen() {
  ...
  if (!trial) {
    return (
      <Screen>
        <Text style={[typeScale.h1, { color: colors.ink }]}>Trial not found</Text>
  ...
  const commit = () => {
    ...
    void tap(correct ? 'success' : 'error');
    setPhase('resolve');
  };
  ...
  return (
    <Screen>
      <Badge
        ...
        tone="gold"
      />
      <Text style={[typeScale.display, { color: colors.ink }]}>{trial.title}</Text>
      <Text style={[typeScale.caption, { color: colors.faint }]}>{trial.kind}</Text>

      <Animated.View entering={FadeIn.duration(300)} key={phase}>
        <Card accent={colors.violet}>
          <Text style={[typeScale.label, { color: colors.gold }]}>GLANCE — FREE SIGNAL</Text>
          <Text style={[typeScale.body, { color: colors.ink }]}>{trial.signal}</Text>
          <Text style={[typeScale.body, { color: colors.ink, fontWeight: '700', marginTop: 4 }]}>
            {trial.prompt}
          </Text>
        </Card>
        ...
        {phase !== 'glance' || probeUsed ? (
          <Card>
            <Text style={[typeScale.label, { color: colors.violetSoft }]}>PROBE SIGNAL</Text>
      ...
      {phase === 'commit' || phase === 'resolve' ? (
        <Card>
          <Text style={[typeScale.label, { color: colors.gold }]}>COMMIT — PICK + CONFIDENCE</Text>
          <View style={{ gap: spacing.sm }}>
              ...
              return (
                <Pressable
                  ...
                  style={{
                    ...
                    backgroundColor: active ? '#2A2111' : colors.surface2,
                    borderRadius: radius.md,
                    padding: spacing.md,
                  }}
                >
                  <Text style={[typeScale.body, { color: colors.ink }]}>{opt}</Text>
                </Pressable>
              );
            })}
          </View>
          <Text style={[typeScale.label, { color: colors.dim, marginTop: spacing.sm }]}>
            CONFIDENCE (FOCUS)
          </Text>
            ...
            {CONF.map((c) => (
              <Pressable
                ...
                style={{
                  ...
                  padding: spacing.sm,
                  alignItems: 'center',
                  backgroundColor: confidence === c.id ? '#2A2111' : 'transparent',
                }}
              >
                <Text style={[typeScale.body, { color: colors.ink, fontWeight: '700' }]}>{c.label}</Text>
                <Text style={[typeScale.caption, { color: colors.dim }]}>{c.hint}</Text>
              </Pressable>
            ))}
          </View>
          {phase === 'commit' ? (
